defmodule AetherATProtoCore.CIDTest do
  ...
  describe "encode/1" do
    test "encodes CID struct with printable hash string" do
      ...
      cid = %AetherATProtoCore.CID{hash: cid_string}

      assert AetherATProtoCore.CID.encode(cid) == cid_string
    end
    ...
    test "encodes CID struct with binary hash (non-printable)" do
      ...
      encoded_result = AetherATProtoCore.CID.encode(cid)
    ...
    test "returns string unchanged when passed a binary string" do
      ...
      assert AetherATProtoCore.CID.encode(cid_string) == cid_string
    end
  ...
  describe "decode/1" do
    test "decodes valid CID string" do
      ...
      assert {:ok, %AetherATProtoCore.CID{}} = AetherATProtoCore.CID.decode(cid_string)
    end

    test "returns error for invalid CID string" do
      assert {:error, :invalid_format} = AetherATProtoCore.CID.decode("invalid_cid")
    end
    ...
    test "decode is alias for parse_cid" do
      ...
      assert AetherATProtoCore.CID.decode(cid_string) ==
               AetherATProtoCore.CID.parse_cid(cid_string)
    end
  end
  ...
  describe "parse_cid_bytes/1" do
    test "parses valid CID from binary data" do
      ...
      assert {:ok, cid, <<>>} = AetherATProtoCore.CID.parse_cid_bytes(cid_string)
      assert %AetherATProtoCore.CID{} = cid
    ...
    test "returns error for invalid CID binary data" do
      ...
      assert {:error, :invalid_cid} = AetherATProtoCore.CID.parse_cid_bytes(invalid_data)
    end

    test "handles empty binary input" do
      assert {:error, :invalid_cid} = AetherATProtoCore.CID.parse_cid_bytes("")
      assert {:error, :invalid_cid} = AetherATProtoCore.CID.parse_cid_bytes(<<>>)
    end
    ...
    test "handles binary data that is not a valid CID" do
      ...
      for invalid <- invalid_inputs do
        assert {:error, :invalid_cid} = AetherATProtoCore.CID.parse_cid_bytes(invalid)
      end
    end
    ...
    test "round trip: cid_to_bytes then parse_cid_bytes" do
      ...
      {:ok, original_cid} = AetherATProtoCore.CID.parse_cid(cid_string)

      # Convert to bytes (assuming cid_to_bytes/1 exists and returns binary)
      bytes = AetherATProtoCore.CID.cid_to_bytes(original_cid)

      # Parse back from bytes
      {:ok, parsed_cid, <<>>} = AetherATProtoCore.CID.parse_cid_bytes(bytes)
    ...
    test "handles binary with extra data (rest parameter)" do
      ...
      assert {:ok, _cid, <<>>} = AetherATProtoCore.CID.parse_cid_bytes(cid_string)
    end
  end
