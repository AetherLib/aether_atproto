defmodule AetherATProtoCore.CAR do
  @moduledoc """
  ...
  # Write a CAR file
  blocks = [
    %AetherATProtoCore.CAR.Block{cid: commit_cid, data: commit_data},
    %AetherATProtoCore.CAR.Block{cid: mst_cid, data: mst_data}
  ]

  car = %AetherATProtoCore.CAR{
    roots: [commit_cid],
    blocks: blocks
  }

  {:ok, binary} = AetherATProtoCore.CAR.encode(car)
  File.write!("repo.car", binary)

  # Read a CAR file
  binary = File.read!("repo.car")
  {:ok, car} = AetherATProtoCore.CAR.decode(binary)
  ```
  ...
  ## Examples
      ...
      iex> block = %AetherATProtoCore.CAR.Block{cid: cid, data: <<1, 2, 3>>}
      iex> car = %AetherATProtoCore.CAR{roots: [cid], blocks: [block]}
      iex> {:ok, binary} = AetherATProtoCore.CAR.encode(car)
  ...
  ## Examples
      ...
      iex> car = %AetherATProtoCore.CAR{roots: [cid], blocks: [block]}
      iex> {:ok, binary} = AetherATProtoCore.CAR.encode(car)
      iex> {:ok, decoded} = AetherATProtoCore.CAR.decode(binary)
      iex> decoded.version
      1
  """
  ...
  ## Examples
      ...
      iex> cid = AetherATProtoCore.CID.parse_cid!("bafyreie5cvv4h45feadgeuwhbcutmh6t2ceseocckahdoe6uat64zmz454")
      iex> block = %AetherATProtoCore.CAR.Block{cid: cid, data: <<1, 2, 3>>}
      iex> car = %AetherATProtoCore.CAR{blocks: [block]}
      iex> {:ok, found} = AetherATProtoCore.CAR.get_block(car, cid)
      iex> found.data
      <<1, 2, 3>>
  """
  ...
  ## Examples
      ...
      iex> block = %AetherATProtoCore.CAR.Block{cid: cid, data: <<1, 2, 3>>}
      iex> car = %AetherATProtoCore.CAR{blocks: [block]}
      iex> blocks = AetherATProtoCore.CAR.list_blocks(car)
      iex> length(blocks)
      1
  """
